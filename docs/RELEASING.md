...
Stage 3 adds automated validation and draft GitHub releases for the Windows
desktop application. The production StreamerSongList API cutover remains a
separate, intentionally deferred phase.
...
3. Runs all tests in Release configuration.
4. Builds the complete solution in Release configuration.
5. Runs the same verified single-file publish script used locally.
6. Retains the resulting executable as a GitHub Actions artifact for 14 days.
...
After the intended commit has passed `Windows CI`, create and push an annotated
tag whose value matches the release version:
...
The `Draft Windows release` workflow repeats the tests, builds the executable
with the tag's version, and creates a draft GitHub release with generated notes.
It does not publish the release automatically. Review the notes, download and
launch the attached executable, then publish the draft from GitHub when ready.
...
- Confirm the release contains exactly `SonglistSpinner.Desktop.exe`.
- Launch the executable on a clean Windows account or test machine.
- Test saving a streamer access token without exposing it in logs or screenshots.
- Test queue loading, spinning, direct mark-as-played, history refresh, reset,
  and the OBS overlay at `http://localhost:5150/overlay`.
